...
> **v0.1.0 — ベータ公開で，頻繁に更新しています．**
> 機能追加・修正・内部構成の変更を日単位で `main` に入れています．
> 手元の版が古いと，この README の説明と挙動が食い違うことがあります．
> **更新は `git pull` → `pip install -r requirements.txt` → サーバー再起動**の 3 手順です
> （[2.8 最新版に更新する](#28-最新版に更新する)）．設定（`.env`）や保存した結果は消えません．
>
...
| Ubuntu / Windows × Python 3.10〜3.13 | **CI で `pytest` 全件が通過**（件数は CI のログを参照．サーバー起動が要る一部は CI では skip）．サーバー側の動作は確認済み |
| ブラウザ UI（Linux / headless Chromium） | **CI でスモークテスト**（読み込み・3 描画モード・統計パネル・エディタ．`tests/e2e`）．フロントの純粋ロジックは node の単体テスト（`tests/js`） |
| ブラウザ UI（Chrome 以外） | **未確認** |
| macOS | **サポート対象外**．検証の予定もありません |

つまり **サーバー側は 3 OS で検証済み，ブラウザ側は Windows / Chrome の実機と Linux / Chromium の自動テスト**という状態です．
...
コストの目安は，1 ターンあたり数円程度です．実際の使用量と概算費用は，サーバーのログに
`usage ...: in=... cache_read=... out=... ~JPY ...` として 1 ターンごとに出るので，
そこで確認できます（画面には出しません）．プロンプトキャッシュが効くよう設計されているため，
...
頻繁に更新しているので，しばらく使っていなければ次の手順で最新版にしてください．
**アンインストールや入れ直しは不要**で，同じフォルダのまま置き換わります．

```bash
cd risu-local
git pull                              # ① コードを最新に
.venv\Scripts\activate                 # （Linux は source .venv/bin/activate）
pip install -r requirements.txt       # ② 依存が増えていることがある
python server.py                      # ③ サーバーを起動し直す（Ctrl+C で止めてから）
```

- **消えないもの**: `.env`（API キーや設定），`RISU_RESULTS_DIR` に保存した結果，
  ブラウザの会話履歴（localStorage）．いずれも `git pull` の対象外です．
- **消えるもの**: サーバーのメモリ上にあった結果（再起動で毎回消えます．残したいものは
  `.json+result` でダウンロードするか [3.6 結果の保存](#36-結果の保存) の永続化を使ってください）．
- **画面が崩れる / 前の表示のままに見える**: ブラウザで `Ctrl+F5`（強制再読み込み）．
  通常は不要ですが，更新直後に古い JS が残ることがあります．
- **`git pull` が「ローカルの変更があるため」で止まる**: 自分で編集したファイルがある状態です．
  変更を残すなら `git stash` → `git pull` → `git stash pop`，捨ててよければ
  `git checkout -- <ファイル>` してから `git pull` してください．
- **依存の入れ直しで壊れた**: 仮想環境を作り直すのが確実です．
  `.venv` を削除して [2.2](#22-python-仮想環境を作る) からやり直してください（`.env` はそのまま使えます）．
- **ZIP でダウンロードして使っている場合**: 新しい ZIP を展開し，古いフォルダから `.env`
  （と結果の保存先）を新しいフォルダにコピーしてください．`.venv` は新しいフォルダで作り直します．
  今後も更新に追随するなら，`git clone` に切り替えるのが手軽です．

何が変わったかは `git log --oneline -20` か，GitHub の
[コミット履歴](../../commits/main) で確認できます．
...
青時間を東西 40 秒・南北 40 秒に変えて，平均旅行時間がどう変わるか比べて
南北の需要を 2 倍にして，どちらの現示を長くすべきか提案して
交差点 I1 の信号を外して常時通行可能にしたら，渋滞はどうなる？
信号交差点が 3 つ並ぶ幹線道路を作って，各交差点の青時間を 45 秒ずつにして
```

信号は交差点ノードの **現示ごとの青時間**（例: `[60, 30]` = 現示 0 が 60 秒，現示 1 が 30 秒）と，
流入道路がどの現示で青になるか（`signal_group`）で表します．黄・全赤の時間はなく，
現示のオフセット（系統制御）は現時点では設定できません．結果の画面では各流入道路の
停止位置に緑 / 赤の停止線バーが出ます（[3.3](#33-結果を見る)）．
青時間の変更や信号の撤去は，作り直さずに「〜に変えて」と続けるだけで差分として反映されます．
...
`ENTERED`（流入台数）が，時刻に連動して更新されます．台数はすべて実台数です
（1 ドット = `deltan` 台）．各値の定義:

| 値 | 定義 |
|---|---|
| `ACTIVE VEHICLES` | その時刻に走行中の台数（描画用の間引き前の全車両） |
| `AVG SPEED` | 走行中の全車両の速度の平均（台数重み）．LLM に渡る `network_avg_speed` と同じ定義 |
| `COMPLETED` / `ENTERED` | 到着・流入の累積台数．実際のイベント時刻から集計し，終了時刻まで正しく更新される |

タイムライン下の波形（混雑度）だけは，リンクごとの速度 / 自由流速度の平均です．

**信号の表示**

信号のある交差点では，各流入道路の停止位置に道路を横切る短い線（**停止線バー**）を描きます．
緑は進入可，赤は停止（小さな三角が進入方向）です．UXsim に黄現示は無いので 2 色だけです．

ズームアウトして流入道路が画面上で短くなると停止線バーは省かれます．
ズームインすると再び表示されます．信号の設定（各現示の青秒）はノードにマウスを
乗せると表示されます．
...
- **リンク** — 名前 / 延長 / 自由流速度 / 車線数 / **容量（台/s）** / 信号 group
  （容量は空欄で「自動（基本図由来）」，`0` は「流出を止める」有効な指定です．
  信号 group は空欄で「その交差点の全現示で青 = 常時通行可能」です）

「**需要**」ボタンで OD 需要（出発地・目的地・時間帯・流量）を編集し，
ツールバー右の **tmax**（シミュレーション終了時刻，秒）・**反応時間**（車頭時間，秒）・
**seed**（乱数シード）を設定して「**▶ 実行**」でそのままシミュレーションできます．
反応時間と seed は元のシナリオの値が引き継がれます（空欄なら UXsim 既定 / 毎回変動）．
道路を少し直して比較するとき，容量の前提や乱数列が勝手に変わることはありません．
...
> **注意**: 既定ではシミュレーション結果はサーバーのメモリ上にあり，**`python server.py` を
> 再起動すると消えます**（最新 30 件まで保持）．残したい結果は
> `.json` / `.json+result` でダウンロードするか，次の永続化を有効にしてください．

**ディスクに残す（任意）**

`.env` に保存先を書くと，実行のたびに結果がそこへ書かれ，再起動後も `sim_id` で開けます
（履歴ボタンからの復元・`rerun_simulation` の base・MCP の `get_result` すべて）:

```ini
RISU_RESULTS_DIR=./results
```

- ファイルは `<sim_id>.json.zst`（`zstandard` が無い環境では `.json.gz`）．中身は
  **`.json+result` でダウンロードするものと同じ**なので，ダウンロード済みの JSON を
  このフォルダに置くだけでも読み込めます（ファイル名が `sim_id` になります）．
- メモリ側は 30 件のキャッシュとして働き，古い結果は必要になったときにディスクから戻ります．
- 個車フレームはダウンロードと同じ量子化（位置 1 m，速度 0.1 m/s）で保存されます．
  統計・台数・累積系列は無損失です．
- ディレクトリ内のファイルは自動では消しません．不要になったら手で削除してください．
...
| `name` | `"sim"` | シミュレーション名 |
| `tmax` | `3600` | 終了時刻（秒） |
| `deltan` | `5` | 車両集計単位（1 台のドットが何台を表すか．小さいほど精密で重い）．統計・ACTIVE VEHICLES・LLM に渡る台数はこの係数を掛けた実台数 |
| `reaction_time` | UXsim 既定 | 車頭時間（秒）．1.5〜1.7 で高速道路の実勢容量（1,800〜2,000 台/時/車線）に近づく |
| `random_seed` | なし | シミュレーション本体の乱数シード．省略すると経路選択ノイズ・合流順が実行ごとに変わり，同じ入力でも平均旅行時間が数十秒ぶれる．条件比較・追試では固定する．保存シナリオ・GUI 再実行・`run_scenario.py --emit` の全経路で維持される |

シミュレーションは確率的です．「容量を変えたら旅行時間が変わった」と結論する前に，
同じ `random_seed` で比較しているか確認してください．画面右上の sim_id の隣に，
現在の結果の deltan・反応時間・seed（と OSM 取込時は需要が自動生成であること）が表示されます．
...
| 全体パラメータ | `{"action":"set_params","reaction_time":1.7,"random_seed":42}`（`tmax` / `deltan` も可）．seed は保存シナリオに残るので，同じ base から派生させる限り以降の rerun でも同じ乱数列になる |
